#!/usr/bin/bash
# See: https://github.com/linuxserver/docker-openssh-server/blob/master/root/etc/s6-overlay/s6-rc.d/init-openssh-server-config/run

# create folders
mkdir -p \
    /config/{.ssh,logs/openssh,sshd}

USERNAME=${USERNAME:-rare}
echo "User name is set to $USERNAME"
